define(['parser', 'underscore'], function(mathFunctionParser, _) {
	...
	return [operator].concat( _.map( operands, function(v,i) { return substitute_ast(v,bindings); } ) );
    };
	...
	var operator = tree[0];
	var operands = tree.slice(1);
	operands = _.map( operands, function(v,i) { return associate_ast(v, op); } );
	...
	return [].concat( _.map( operands, function(v,i) { return leaves(v); } ) );
    }
	...
	if (operator in math_functions) {
	    return "(" + text_functions[operator]( _.map( operands, function(v,i) { return text_ast(v); } ) ) + ")";
	}
	...
	if (operator === '*') {
		...
		return [['*'].concat( _.map( inner_operands, function(w,j) {
		    if (i == j) {
